...
for train_index, valid_index in index_split: 
    thisFold = thisFold + 1
    if thisFold <=5:                                              ############# DELETE THIS 
        print("Start fold: %s" % thisFold)
        temp_name = str(thisFold) + '_fold_index.csv'
        pd.DataFrame({"index": valid_index}).to_csv(temp_name,index=False) # save the fold index to files 
        continue
    
    if thisFold == 1:                                                  # prepare the y_train as followed by index 
...
final_train_X.to_csv('stacking_1.0.csv', index = False) 

print('\n\nstart to create stacking for test ...')
...
for b in range(sgd_param['BAG_SIZE']):
    sgd = SGD(loss=sgd_param['loss'], 
              ...
              n_iter=sgd_param['n_iter'],
              verbose=sgd_param['verbose'])
    sgd.fit(X_train_min, X_test_std_min)
